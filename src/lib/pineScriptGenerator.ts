...
if i_debugTable and barstate.islast
    table.cell(dbg, 0, 0, "UI→Pine", bgcolor=color.new(color.blue, 75), text_color=color.white)
    ...
    table.cell(dbg, 0, 1, "Active filters")
    table.cell(dbg, 1, 1, activeFilters)
    table.cell(dbg, 2, 1, activeFilters)
    table.cell(dbg, 3, 1, "strategy=" + f_on(i_strategyOn))
    table.cell(dbg, 0, 2, "Soft count")
    table.cell(dbg, 1, 2, str.tostring(longSoftCount) + "/" + str.tostring(requiredSoft))
    table.cell(dbg, 2, 2, str.tostring(shortSoftCount) + "/" + str.tostring(requiredSoft))
    table.cell(dbg, 3, 2, "active=" + str.tostring(softSlotCount))
    table.cell(dbg, 0, 3, "Requested soft")
    table.cell(dbg, 1, 3, str.tostring(i_requestedSoft))
    table.cell(dbg, 2, 3, "Effective")
    table.cell(dbg, 3, 3, str.tostring(requiredSoft))
    table.cell(dbg, 0, 4, "Soft warning")
    table.cell(dbg, 1, 4, softClampWarning ? "CLAMPED" : "none", text_color=softClampWarning ? color.yellow : color.white)
    table.cell(dbg, 2, 4, "")
    table.cell(dbg, 3, 4, softClampWarning ? "requested > active" : "ok")
    table.cell(dbg, 0, 5, "Hard gate")
    table.cell(dbg, 1, 5, f_pass(longHardGate), text_color=longHardGate ? color.lime : color.red)
    table.cell(dbg, 2, 5, f_pass(shortHardGate), text_color=shortHardGate ? color.lime : color.red)
    table.cell(dbg, 3, 5, "")
    table.cell(dbg, 0, 6, "Blockers")
    table.cell(dbg, 1, 6, longBlockers == "" ? "none" : longBlockers, text_color=longBlockers == "" ? color.lime : color.red)
    table.cell(dbg, 2, 6, shortBlockers == "" ? "none" : shortBlockers, text_color=shortBlockers == "" ? color.lime : color.red)
    table.cell(dbg, 3, 6, "")
    table.cell(dbg, 0, 7, "Final signal")
    table.cell(dbg, 1, 7, f_pass(finalLongSignal), text_color=finalLongSignal ? color.lime : color.gray)
    table.cell(dbg, 2, 7, f_pass(finalShortSignal), text_color=finalShortSignal ? color.lime : color.gray)
    table.cell(dbg, 3, 7, rawLongSignal and rawShortSignal ? "tie-break" : "")
    table.cell(dbg, 0, 8, "MACD")
    table.cell(dbg, 1, 8, "hist=" + f_pass(macdHistPassed) + " dir=" + f_pass(longMacdDirPassed))
    table.cell(dbg, 2, 8, "hist=" + f_pass(macdHistPassed) + " dir=" + f_pass(shortMacdDirPassed))
    table.cell(dbg, 3, 8, "mom=" + f_pass(macdMomPassed))
    table.cell(dbg, 0, 9, "HTF / ATR")
    table.cell(dbg, 1, 9, "HTF=" + f_pass(longHtfPassed) + " ATR=" + f_pass(atrPassed))
    table.cell(dbg, 2, 9, "HTF=" + f_pass(shortHtfPassed) + " ATR=" + f_pass(atrPassed))
    table.cell(dbg, 3, 9, "ATR%=" + str.tostring(atrPct, "#.####"))
    table.cell(dbg, 0, 10, "Exits")
    table.cell(dbg, 1, 10, "BE=" + f_on(i_useBE) + " TR=" + f_on(i_useTrail) + " Peak=" + f_on(i_usePeak))
    table.cell(dbg, 2, 10, "MFE=" + f_on(i_useMfeCap) + " Time=" + f_on(i_maxDurMin > 0))
    table.cell(dbg, 3, 10, "auto=" + f_on(i_autoExit))
    table.cell(dbg, 0, 11, "Stops active")
    table.cell(dbg, 1, 11, "BE=" + f_on(beActive) + " TR=" + f_on(trailActive))
    table.cell(dbg, 2, 11, "Peak=" + f_on(peakActive) + " MFE=" + f_on(mfeActive))
    table.cell(dbg, 3, 11, "CondTime=" + f_on(i_condTimeExit))
    table.cell(dbg, 0, 12, "Stoch")
    table.cell(dbg, 1, 12, f_pass(longStochPassed))
    table.cell(dbg, 2, 12, f_pass(shortStochPassed))
    table.cell(dbg, 3, 12, "K=" + str.tostring(kLine, "#.##") + " D=" + str.tostring(dLine, "#.##"))
    table.cell(dbg, 0, 13, "Date range")
    table.cell(dbg, 1, 13, f_pass(inDateRange))
    table.cell(dbg, 2, 13, "")
    table.cell(dbg, 3, 13, "")
`;
}
